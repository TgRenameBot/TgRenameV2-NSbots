...
async def media(c, m):
    """Checking the File & Processing the renaming"""
    ...
    file_name = await c.ask(chat_id=m.from_user.id, text="Send me the New FileName with extension for this file or send /cancel to stop", filters=filters.text)
    ...
    try:
        file_location = await m.download(
                            ...
                            progress_args=("Uploading to Server:", start_time, send_message)
                        )
    except Exception as e:
        logger.error(e)
    ...
    if as_file:
        try:
            await m.reply_document(
                ...
                thumb=thumbnail,
                progress=progress_bar,
                progress_args=("Uploading to TG:", start_time, send_message)
            )
...
async def notify(m, time_gap):
    ...
    await m.reply_text("__You can send another file Now__")
